function right_stance_constraints(nlp, bounds, varargin)
    ...
    Body2ToeDistance = SymFunction(['Body2ToeDistance_',domain.Name], sqrt(sum((p_stToe-p_body).^2)), {domain.States.x});
    ...
    Toe2ToeDistance = SymFunction(['Toe2ToeDistance_',domain.Name], sqrt(sum((p_RightToe(1:2)-p_LeftToe(1:2)).^2)), {domain.States.x});
